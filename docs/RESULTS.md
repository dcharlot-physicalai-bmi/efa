...
| emergent commitment (does bistability arise, unforced?) | **clean controlled null.** With a *learnable* well depth A on an early-reliable/late-noise task, commitment does **not** emerge (cold-start A stays ~0.05) — and the decisive control: a **warm-started** well (A=0.60) *decays to 0.07*, i.e. training throws away an imposed well because it buys nothing (all models ~91–93% across K). So v3's commitment must be **imposed**, not learned here. In energy terms: the well is *energetically unjustified* — K=1 already suffices, so there are no watts to save. (Open: commitment should emerge only where it *saves* energy — a task needing high K for a smooth solver but low K for a committed one.) |
