...
| harder body — double-pendulum energy (invariant discovery) | **chaotic** double pendulum (4-D, strongly coupled) | **honest, well-characterized negative.** Physics verified exact (a consistency check caught a mis-coded flow — energy drift 13.4 → fixed to **0.0000**). But recovering the conserved energy *from trajectories* — trivial on the single pendulum (**corr 0.99**) — resisted **three** principled objectives on the chaotic body (conservation-residual, and trajectory-variance with/without a total-variance constraint), all landing at **\|corr\|≈0.03–0.11**: limited trajectories under-constrain the invariant and the objectives find degenerate solutions. It's an ill-posed inverse problem; the correct next method is an **HNN in canonical coordinates** (learn H(q,p) matching Hamilton's equations via the mass matrix) — identified, not yet built. |
