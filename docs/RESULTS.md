...
| harder body — double-pendulum energy (invariant discovery) | **chaotic** double pendulum (4-D, strongly coupled) | **honest negative.** I first mis-coded the equations (energy drifted 13.4 along the flow); fixed them (drift **0.0000**, physics now exact). But recovering the conserved energy by conservation-residual minimization — which nailed the single pendulum at **corr 0.99** — collapses to **\|corr\|=0.11** on the chaotic body. The method does **not** scale to chaos; the physics is right, the *learning* fails. |
| full cart-pole regulation (cart-centre + balance) | cart-pole, both regulated | **unresolved wall.** θ-only balance works (pole RMS 0.010, cart drifts). Adding cart-centering: the stable region for *simultaneous* cart+pole regulation is narrow (cart drifts unless cart gains are strong; strong cart gains destabilize the pole) and neither hand-tuning nor a coarse 4-D grid search found it. Needs proper **LQR (Riccati)**, deferred. Reported, not hidden. |
