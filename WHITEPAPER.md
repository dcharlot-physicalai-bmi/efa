...
The loop closes on the sensing side too, and on a harder body. On a **cart-pole** (4-D, underactuated) the controller is given only **noisy position observations** — no velocities — so it must *infer* the state, which is the EFA reading of perception: inference to a low-energy explanation. An energy observer (the minimizer of observation-mismatch + a learned-dynamics prior — trust the smooth model prediction, gently correct with the position innovation) recovers velocities cleanly and **balances** the pole, while the obvious baseline — velocities by differencing the noisy positions — **fails catastrophically** at the same noise (its velocity estimate is pure amplified noise). Same body, same controller, same noise; only the perception front-end differs. Perception, control, and dynamics are all the same energy object — which is why the embodied loop closes on-device. *(Contrast: a discrete-token puzzle like the multivalued-algebra toy in §3.4 does **not** test this — a plain feedforward solves it; the architectural edge lives in continuous, force-governed dynamics, not toy capability.)*
