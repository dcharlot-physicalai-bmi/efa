...
**Grounding — the physics of intelligence.** This is not only an engineering position; it aligns with a principled account of what intelligence *is*. In the complexity-science framing (Krakauer, SFI), the physical world runs on a least-action functional; **adaptation** adds *endogenized memory* to that functional; **agency** adds *the future* through a policy. EFA is that ladder made computational — one energy (the least-action object) that predicts by descent, remembers through fast-weights written at inference, and plans by descending toward a goal. And intelligence, on this account, is **making hard problems easy**: the expert *looks less*, having recoded the problem into a representation where the answer is a reflex (system-2 → system-1). That is exactly *joules per task* — capability bought by better *representation*, not more search. Two consequences fall out and are measured in this document: intelligence should show as **economy of effort** (the compute to solve *falls* as the energy landscape recodes, §3.7), and where the problem is embedded in the physical world, **materiality does computation** — the material drops the search clauses, so descent on a physically-structured energy solves what abstract search cannot (§3.7). The upshot: EFA's edge is not a bigger model but a *better-shaped energy* — the transparent map, where the cloud LLM is an opaque GPS.
...
### 3.7 The physics of intelligence — economy of effort, and materiality
Two measured consequences of the Krakauer grounding (§1), each a clean result:
- **Intelligence = economy of effort.** As the descent-trained EBT learns the multivalued system, the descent steps *K* needed to solve **fall**: steps-to-90% drop from 8 to 4, and at a fixed low budget accuracy climbs sharply across training (K=4: 11→100%; K=2: 3→76%). The novice cannot solve at any K; the expert solves at low K. The energy landscape recodes so the hard problem becomes easy — capability bought by *representation*, paid in fewer watts. Honest bound: K=1 never solves; expertise drives compute toward the task's *intrinsic* minimum, not to zero.
- **Materiality does computation.** On disc-packing, abstract random search finds a valid (non-overlapping) configuration in an *exponentially vanishing* fraction of samples as density rises (59% → 0.13% → **0%** at 28% density), while descent on a repulsion energy — the material constraint built into the dynamics — solves **100%** of them in a handful of steps (1→17). The physics drops the "no-overlap" clauses by construction — exactly Krakauer's "physics does for the Soma cube what algorithms do for the Rubik's cube."
...
### 3.9 Frontier check (2026) — recipe corrections, under test
A whitepaper written from a fixed knowledge cutoff risks implementing a sound thesis with a stale recipe. A fresh, adversarially-verified survey of the 2025–26 literature (23 primary sources; 21 claims confirmed by 3-vote verification) forced three corrections — each then tested on the same nano bodies. The corrections *strengthen* EFA by separating what the energy is *for*.

1. **Actuation is flow-matching, not iterative energy descent.** Producing an action by iterative gradient descent on a scalar energy is Implicit Behavior Cloning (IBC), which the field has *quantified as failing* on multi-DOF manipulation (0.21 success vs 0.88 for Diffusion Policy; 0.00 on several tasks) and moved past — toward flow-matching / one-step policies (SSCP, MIP). This is exactly the wall our own multi-DOF descent hit: on a coupled **2-DOF** arm, iterative energy-descent control reached **0%** three different ways. Rebuilt as a **flow-matching** policy — a learned velocity field integrated a *single forward pass* — it reaches **100%** on the identical arm, matching a discrete planner at **1 forward pass vs its 25–81 action-evaluations** (constant-in-DOF vs exponential). Flow-matching *can* be made energy-first by parameterizing the velocity as **−∇E** of a scalar potential (Energy/Equilibrium Matching), which recovers the explicit energy for free; our first cut used a plain velocity net. Building the constrained version and *measuring* it: the one potential **verifies at 95%** and actuates at **46% (rising 17→26→46 with K — a true accuracy-vs-joules curve on a single object)**, while the unconstrained net actuates at 100% — so **unification is real but currently costs actuation**, a fit gap (the ideal per-slice flow field *is* a gradient field), not a fundamental one. The resolution is the **hybrid v = −κ∇ₐE + w** — the potential carries the field, a small ℓ2-penalized correction absorbs the scalar-fit residual — which restores **100% actuation at a single gradient step while the same potential verifies at 94%**, with the potential carrying a measured **65%** of the field. The recipe changed; the thesis need not — and every remaining gap is measured, not hidden.
2. **The stability certificate is earned by contraction + bound propagation, not a learned Lyapunov net.** A directly-learned neural-Lyapunov certificate returned **0%** certified region in our hands. The 2026 route — a **contraction** metric verified over a region (σmax of the metric-weighted closed-loop Jacobian < 1), the scalable method that reaches 10-state systems curse-of-dimension-free — earns a real one: **30.4% certified contraction region, a certified region-of-attraction ball, and 100% empirical convergence from it**. The port-Hamiltonian structural certificate (§3.5, dE/dt = −r·ω² ≤ 0 *by construction*) remains the by-construction special case for mechanical bodies; the generalized (average-over-steps) Lyapunov condition dissolves the "must decrease every single step" worry.
3. **"One scalar energy for everything" → "one shared latent + a small coordinated family of energies."** A Bellman *value* is a poor validity classifier — the §3.8 flagship's *verify* was the weak reading at 76%. Split it: a flow-matching policy *generates* the action and a purpose-built, contrastively-trained energy *verifies* it, on shared features. Measured: actuation **100%**, and **verify jumps to 99.6%** (ranking the good action below a bad one), with the pair coordinating (the flow's own action scored valid 80%). Generate-then-check on one latent is more honest *and* more capable than a lone scalar doing every job.

**What this repositions.** With architecture held fixed, energy/generative control policies reach *parity* with plain regression on behavior-cloning benchmarks — so EFA's edge is **not** "energy descent controls better." It is the intersection the field still leaves open: a **physics-structured** energy that is **certified** (scalably) and **measured in joules-per-task on the edge**. (Two often-cited EBT-Policy claims — emergent zero-shot recovery, and consistently beating diffusion at lower compute — did not survive adversarial verification; treated here as one promising result, not the reference SOTA.) The through-line of §3 holds and sharpens: the lever is the *right structure and the right recipe*, matched to the task — validated by three independent corrections that each turned a wall into a result.
...
- **Recipe, corrected under test (§3.9).** An earlier version of this program used *iterative energy descent over actions* as the control primitive; on multi-DOF it failed (0%), and a 2026 frontier check identified it as Implicit Behavior Cloning — a known-failing recipe — corrected to flow-matching (which then reached 100%). The lesson is priced into the method: *the thesis was sound but the actuation recipe was stale*, and periodic checks against the live frontier are part of the discipline, not an afterthought. Actuation is now flow-matching; the explicit energy is scoped to verify/likelihood/planning and to the certificate — where it is the right object.
